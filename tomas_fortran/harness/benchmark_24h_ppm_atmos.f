C     **************************************************
C     *  TOMAS 24-Hour PPM Benchmark — Atmospheric 1000 *
C     **************************************************
C
C     Scaled-up variant of benchmark_24h_ppm.f for a 1000-scenario
C     tropo -> strato LHC.  Reads scenarios_atmos.csv from the working
C     directory; writes outputs to output/atmos/ with 4-digit scenario
C     IDs (ppm_sNNNN_...).
C
C     Modes: 1=coag, 2=cond (PPM), 3=combined
C
C     Output: CSV files in output/atmos/ directory
C       ppm_sNNNN_{coag|cond|combined}_hour{01-24}_{Nk|Mk|Gc}.csv
C       timing_fortran_ppm_atmos.csv
C
C     Progress print every 25 scenarios to keep stdout manageable.

      PROGRAM benchmark_24h_ppm_atmos
...
C-----VARIABLE DECLARATIONS-------------------------------------------
      ...
      parameter(nscenarios=1000, nsteps=1440, nhours=24)
...
C-----READ SCENARIOS---------------------------------------------------
      ...
      write(*,*) '========================================'
      write(*,*) 'TOMAS 24-Hour PPM Benchmark — 1000-Atmos'
      write(*,*) '========================================'
      write(*,*) 'Reading scenarios from scenarios_atmos.csv ...'

      open(unit=20, file='scenarios_atmos.csv', status='old',
     &     form='formatted')
...
C-----MAIN LOOP: scenarios x modes-------------------------------------
      ...
      do iscen=1,nscenarios
         if (mod(iscen, 25) .eq. 1 .or. iscen .eq. nscenarios) then
            write(*,'(A,I5,A,I5)')
     &           ' Scenario ', iscen, '/', nscenarios
            call flush(6)
         endif
...
C              --- Hourly output (ppm_ prefix, 4-digit scenario) ---
               if (mod(istep, 60) .eq. 0) then
                  ihour = istep / 60

C                 Write Nk
                  write(fname,'(A,I4.4,A,A,A,I2.2,A)')
     &                 'output/atmos/ppm_s',iscen,'_',
...
C        Write timing CSV incrementally (crash-tolerant: flush per scenario)
         if (iscen .eq. 1) then
            open(unit=30,
     &           file='output/atmos/timing_fortran_ppm_atmos.csv',
     &           status='replace')
     ...
     &           position='append')
         ...
         write(30,'(I5,A,F12.4,A,F12.4,A,F12.4)')
     &        iscen, ',',
     &        scen_times(iscen,1), ',',
     &        scen_times(iscen,2), ',',
     &        scen_times(iscen,3)
      ...
      write(*,*) ''
      write(*,*) '========================================'
      write(*,*) '1000-Atmos PPM benchmark complete!'
      write(*,*) 'Output in output/atmos/'
      write(*,*) 'Timing in output/atmos/timing_fortran_ppm_atmos.csv'
      write(*,*) '========================================'
